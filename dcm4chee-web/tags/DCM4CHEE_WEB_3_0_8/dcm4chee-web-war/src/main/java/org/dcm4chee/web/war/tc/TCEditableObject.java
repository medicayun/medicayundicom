...
public class TCEditableObject extends TCObject {
    ...
    private SaveResult save(boolean saveLinkedCases) throws Exception {
    	if (modified) {
	        ...
	        //now save this case
	    	if ( TCStoreDelegate.getInstance().modifyInstanceImmediately( toInstanceModificationDataset() ) &&
	    			TCStoreDelegate.getInstance().modifySeriesImmediately( toSeriesModificationDataset() ) ) {
    			modified = false;
    ...
    private DicomObject toSeriesModificationDataset()
    {
        BasicDicomObject modification = createDicomObject();
        
        modification.putString(Tag.SeriesInstanceUID, VR.UI, getSeriesInstanceUID() );
        modification.putString( Tag.SeriesDescription, null, getTitle() );
        
        return modification;
    }
    
    private DicomObject toInstanceModificationDataset()
    {
        BasicDicomObject modification = createDicomObject();
        
        modification.putString(Tag.SOPInstanceUID, VR.UI, getInstanceUID());
        modification.putString(Tag.SOPClassUID, VR.UI, getClassUID());
        
        //proprietary; used to show some useful info in the GUI when doing c-find
        modification.putString(Tag.ContentLabel, null, getTitle());
        modification.putString(Tag.ContentDescription, null, getAbstr());
        modification.putString(Tag.ContentCreatorName, null, getAuthorName());
        ...
        if ( (docsRemoved!=null && docsRemoved.size()>0) ||
        	...
        	if (refs!=null) {
        		modification.add(refs);
        		refs = modification.get(Tag.CurrentRequestedProcedureEvidenceSequence);
        	}
        	...
        	if (docsAdded!=null && !docsAdded.isEmpty()) {
	        	if (refs==null) {
	        		refs = modification.putSequence(Tag.CurrentRequestedProcedureEvidenceSequence);
	        	}
        ...
        // recompile content
        DicomElement content = modification.putSequence(Tag.ContentSequence); //add new content
    ...
    {
        DicomObject ds = createDicomObject();
        ...
        ds.putString(Tag.TextValue, VR.UT, text);
        return ds;
    }
    ...
    private BasicDicomObject createDicomObject()
    {
    	BasicDicomObject dataset = new BasicDicomObject();
    	
    	if ( this.ds.containsValue( Tag.SpecificCharacterSet ) )
    	{
    		dataset.putString( Tag.SpecificCharacterSet, VR.CS, this.ds.getString( Tag.SpecificCharacterSet ) );
    	}
