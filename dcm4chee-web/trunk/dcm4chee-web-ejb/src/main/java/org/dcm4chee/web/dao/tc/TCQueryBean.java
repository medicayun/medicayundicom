...
public class TCQueryBean implements TCQueryLocal {
	...
	public Map<String, Integer> findMultiframeInstances(String stuid)
    {
    	if (stuid!=null)
    	{
	    	...
	    	// compile result
	    	return findMultiframeInstances(result, null);
    	}
    ...
    @SuppressWarnings("unchecked")
	public Map<String, Integer> findMultiframeInstances(String stuid, String suid, Set<String> iuids)
    {
    	if (iuids!=null && !iuids.isEmpty())
    	{
    		// compile HQL query string
	    	StringBuilder queryString = new StringBuilder("FROM Instance i WHERE i.series.study.studyInstanceUID=:stuid AND i.series.seriesInstanceUID=:suid");
	    	...
	    	// compile result
	    	return findMultiframeInstances(result, iuids);
    	}
    ...
    private Map<String, Integer> findMultiframeInstances(List<Instance> instances, Set<String> iuids)
    {
    	if (instances!=null && !instances.isEmpty())
    	{
    		HashSet<String> filterSet = iuids == null ? new HashSet<String>(0) :
    				iuids instanceof HashSet ? (HashSet<String>) iuids :
    					new HashSet<String>(iuids);

    		Map<String, Integer> map = new HashMap<String, Integer>();
    		for (Instance i : instances)
    		{
    			if ( iuids == null || filterSet.contains( i.getSOPInstanceUID() ) )
    			{
