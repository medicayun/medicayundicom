...
public class TCStoreDelegate extends BaseMBeanDelegate 
{
    ...
    public boolean modifyInstanceImmediately(DicomObject dataset) throws Exception {
    	return storeImmediately(UID.Dcm4cheAttributesModificationNotificationSOPClass, 
    			createModificationDataset(dataset, "IMAGE"));
    }
    
    public boolean modifySeriesImmediately(DicomObject dataset ) throws Exception {
    	return storeImmediately(UID.Dcm4cheAttributesModificationNotificationSOPClass, 
    			createModificationDataset(dataset, "SERIES"));
    }
    ...
    private DicomObject createModificationDataset(DicomObject dataset, String qrLevel) {
    	BasicDicomObject ds = new BasicDicomObject();
    	dataset.copyTo(ds);
    	
        //specific AttributeModificationService attributes
        ds.putString(Tag.QueryRetrieveLevel, VR.CS, qrLevel);
