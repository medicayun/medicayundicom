...
*/    
    ...
    private FileFormat detectFileFormat(byte[] b) throws DcmParseException, IOException
    {
        ...
        // Note: The first few bytes can be a private tag or private attribute when parsing
        // a DICOM header fragment, e.g. a fragment stored in the various xx_attrs columns
        // in the database.
        // Validate VR for three types of tags:
        // - non-private tag: VR must match the expected one for that tag
        // - private creator data element: VR must be LO
        // - private tag: VR must be valid
        if ((VRMap.DEFAULT.lookup(tag) == vr) || (Tags.isPrivateCreatorDataElement(tag) && vr == VRs.LO) || 
                (Tags.isPrivate(tag) && VRs.isValid(vr))) {
            return fmi
                    ? (bigEndian 
