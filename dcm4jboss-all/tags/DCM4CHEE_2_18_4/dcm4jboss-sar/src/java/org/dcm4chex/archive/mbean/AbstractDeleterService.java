...
public abstract class AbstractDeleterService extends ServiceMBeanSupport {
    ...
    public abstract String getMinFreeDiskSpace();
    ...
    public long getUsableDiskSpace() throws Exception {
        return calcUsableDiskSpace(getMinFreeDiskSpace());
    }
    
    public long calcUsableDiskSpace(String minFree) throws Exception {
        return calcUsableDiskSpace(fileSystemMgt().getFileSystemsOfGroup(getFileSystemGroupIDForDeleter()), minFree);
    }

    private long calcUsableDiskSpace(FileSystemDTO[] fsDTOs, String minFree) throws IOException {
        boolean allRW = getMinFreeDiskSpaceBytes() != 0;
        long free = 0L;
        long curMinFree;
        boolean isPercent = minFree.endsWith("%");
        Number minFreeValue = isPercent ? Float.valueOf(minFree.substring(0,minFree.length()-1)) / 100.0f : getMinFreeDiskSpaceBytes(); 
        for (FileSystemDTO fsDTO : fsDTOs) {
            int status = fsDTO.getStatus();
            if ( (allRW && status == FileSystemStatus.RW) || status == FileSystemStatus.DEF_RW) {
                File dir = FileUtils.toFile(fsDTO.getDirectoryPath());
                if (dir.isDirectory()) {
                	curMinFree = isPercent ? (long)(FileSystemUtils.totalSpace(dir.getPath()) * minFreeValue.floatValue()) : minFreeValue.longValue();
                    free += Math.max(0, FileSystemUtils.freeSpace(dir.getPath()) - curMinFree);
                }
            }
        }
        return free;
    ...
    private int scheduleStudiesForDeletion(FileSystemMgt2 fsMgt,
        ...
        do {
            DeleteStudyOrdersAndMaxAccessTime deleteOrdersAndAccessTime = 
                    fsMgt.createDeleteOrdersForStudiesOnFSGroup(
                            ...
                            notAccessedAfter,
                            scheduleStudiesForDeletionBatchSize - countStudies,
        ...
        } while (countStudies < scheduleStudiesForDeletionBatchSize && sizeToDel > 0 && isRunningScheduleStudiesForDeletion);
        if (countStudies == 0 && sizeToDel > 0) {
            ...
            log.debug("Scheduled "+countStudies+" studies to delete "+FileUtils.formatSize(sizeToDel0-sizeToDel)+
                    ". (requested size to delete:"+FileUtils.formatSize(sizeToDel0)+")");
        }
        return countStudies;
    }
