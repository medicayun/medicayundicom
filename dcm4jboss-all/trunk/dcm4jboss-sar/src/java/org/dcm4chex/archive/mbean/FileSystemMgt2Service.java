...
public class FileSystemMgt2Service extends AbstractDeleterService {
	...
	public String getCurrentStorageFilesystem() {
		return storageFileSystem == null ? "" : storageFileSystem.getDirectoryPath();
	}
    ...
    public long getMinFreeDiskSpaceBytes() {
        if (minFreeDiskSpaceRatio > 0 && storageFileSystem == null) {
        	try {
        		storageFileSystem = fileSystemMgt().getDefRWFileSystemsOfGroup(getFileSystemGroupID());
        		minFreeDiskSpace = this.calcFreeDiskSpace(storageFileSystem);
			} catch (Exception e) {
				log.error("Select storage filesystem failed!", e);
			}
        }
