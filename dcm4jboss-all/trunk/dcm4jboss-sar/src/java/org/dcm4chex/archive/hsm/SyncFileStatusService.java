 ...
 * @version $Revision: 18669 $ $Date: 2016-11-15 22:57:57 +0800 (周二, 15 11月 2016) $
 * @since Nov 22, 2005
 */
public class SyncFileStatusService extends ServiceMBeanSupport {
    ...
    private int maxConcurrentFilechecks = 5;
    ...
    public int getMaxConcurrentFilechecks() {
		return maxConcurrentFilechecks;
	}

	public void setMaxConcurrentFilechecks(int maxConcurrentFilechecks) {
		if (this.maxConcurrentFilechecks != maxConcurrentFilechecks) {
			if (isRunning) {
				throw new RuntimeException("SyncFileStatus is running! Can not change MaxConcurrentFilechecks.");
			}
			this.maxConcurrentFilechecks = maxConcurrentFilechecks;
			this.semaphore = maxConcurrentFilechecks < 2 ? null : new FIFOSemaphore(maxConcurrentFilechecks);
		}
	}
    ...
    public int check() throws Exception  {
        ...
        try {
            ...
            final FileSystemMgt2 fsmgt = newFileSystemMgt();
            FileDTO[] filesToCheck = fsmgt.findFilesByStatusAndFileSystem(fileSystem, checkFileStatus, this.oldestCreatedTimeOfCheckFileStatus,
                    new Timestamp(System.currentTimeMillis() - minFileAge), limitNumberOfFilesPerTask);
            if (log.isDebugEnabled()) log.debug("found "+filesToCheck.length+" files to check status.");
            if (filesToCheck.length == 0) {
                return 0;
            }
            Map<String, List<FileDTO>> filesPerTar = sortByTarfile(filesToCheck);
            final int[] count = new int[]{0};
            final Map<String, Integer> checkedTarsStatus = Collections.synchronizedMap(new HashMap<String, Integer>());
            final Map<String, Map<String, byte[]>> checkedTarsMD5 = verifyTar ? Collections.synchronizedMap(new HashMap<String, Map<String, byte[]>>()) : null;
            final List<Long> failedFilesystems = Collections.synchronizedList(new ArrayList<Long>());
            for (Map.Entry<String, List<FileDTO>> e : filesPerTar.entrySet()) {
            	final List<FileDTO> dtos = e.getValue();
            	final String fn = e.getKey();
                if (failedFilesystems.size() > 0)
                        break;
                if (semaphore == null) {
                	checkFile(fsmgt, count, checkedTarsStatus, checkedTarsMD5, failedFilesystems, dtos);
                } else {
                	log.info("acquire semaphore to check file "+fn+": permits:"+semaphore.permits());
                	semaphore.acquire();
                	log.info("Start SyncFileStatus_checkFile Thread to check file "+fn);
                	new Thread(new Runnable() {
                        public void run() {
                        	try {
                        		checkFile(fsmgt, count, checkedTarsStatus, checkedTarsMD5, failedFilesystems, dtos);
                        	} finally {
                        		log.info("Release semaphore for "+fn);
                        		semaphore.release();
                        	}
                        }
                    }, "SyncFileStatus_checkFile_"+fn).start();
                }
            }
            for (int i = 0 ; semaphore != null && semaphore.permits() < this.maxConcurrentFilechecks ; i++) {
            	if (i%10 == 0)
            		log.info("Wait for "+(maxConcurrentFilechecks - semaphore.permits())+" outstanding file checks!");
            	Thread.currentThread().sleep(500);
            }
            log.info("SyncFileStatus finished in "+(System.currentTimeMillis()-t1)+"ms! changed files:"+count[0]);
            return count[0];
        } finally {
            isRunning = false;
        }
    ...
    private Map<String, List<FileDTO>> sortByTarfile(FileDTO[] c) {
    	Map<String, List<FileDTO>> map = new HashMap<String, List<FileDTO>>(c.length);
    	String fn;
    	for (FileDTO dto : c) {
    		fn = dto.getFilePath();
    		if (dto.getDirectoryPath().startsWith("tar:")) {
   			...
   			List<FileDTO> dtos = map.get(fn);
   			if (dtos == null) {
   				dtos = new ArrayList<FileDTO>();
   				map.put(fn,  dtos);
   			}
   			dtos.add(dto);
    	}
	...
	private boolean check(FileSystemMgt2 fsmgt, FileDTO fileDTO,
            Map<String, Integer> checkedTarsStatus, Map<String, Map<String, byte[]>> checkedTarsMD5) throws IOException, VerifyTarException, HSMException {
    ...
    private Integer verifyTar(FileDTO dto, String tarPathKey, Map<String, Map<String, byte[]>> checkedTarsMD5) {
	...
	private void checkFile(final FileSystemMgt2 fsmgt, final int[] count,
			final Map<String, Integer> checkedTarsStatus, final Map<String, Map<String, byte[]>> checkedTarsMD5,
			final List<Long> failedFilesystems, final List<FileDTO> dtos) {
		for (FileDTO dto : dtos) {
			try {
			    if (check(fsmgt, dto, checkedTarsStatus, checkedTarsMD5))
			        ++count[0];
			} catch (HSMException x) {
			    if (x.getErrorLevel() == HSMException.ERROR_ON_FILESYSTEM_LEVEL) {
			        failedFilesystems.add(dto.getFileSystemPk());
			        log.warn("Check of file "+dto.getFilePath()+" failed on filesystem level! skip all files of "+
			                dto.getDirectoryPath()+" for this request!");
			    }
			} catch (Exception x) {
			    log.warn("Check of file "+dto.getFilePath()+" failed! skipped!");
			}
		}
	}
}
