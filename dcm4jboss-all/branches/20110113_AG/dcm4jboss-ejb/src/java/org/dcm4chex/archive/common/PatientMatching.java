...
public class PatientMatching implements Serializable{
    ...
    private PatientMatching(int pid, int issuer, String s) {
        ...
        if (alt != -1) {
            altDemographicsMatch = 
                    new PatientMatching(extractAltExpression(alt, s));
            s = s.substring(0, alt);
        }
    ...
    String extractAltExpression(int altPos, String originalExpression) {
    	String originalPidIssuer = extractPidAndIssuerExpression(originalExpression);
        String altExpression = originalExpression.substring(altPos + 1);
        String altPidIssuer = extractPidAndIssuerExpression(altExpression);
        if ( altPidIssuer.length() > 0 && !originalPidIssuer.equals(altPidIssuer) ) {
        	throw new IllegalArgumentException("Mismatched pid and issuers are not allowed in alternate expressions: " +
        			originalPidIssuer + " != " + altPidIssuer);
        }
        return altExpression.startsWith(originalPidIssuer) ? altExpression : originalPidIssuer + altExpression;
    }
    
    private String extractPidAndIssuerExpression(String expression) {
    	int pidPos = indexOf(expression, PID);
    	StringBuilder pidAndIssuerExpression = new StringBuilder();
		if ( pidPos != -1 ) {
	        pidAndIssuerExpression.append(PID);
	        if ( unknownAlwaysMatch(expression, pidPos, PID, false) ) {
	        	pidAndIssuerExpression.append('?');
	        }
	        pidAndIssuerExpression.append(',').append(ISSUER);
	        if ( unknownAlwaysMatch(expression, indexOf(expression, ISSUER), ISSUER, false) ) {
	        	pidAndIssuerExpression.append('?');
	        }
	        pidAndIssuerExpression.append(',');
		}
		return pidAndIssuerExpression.toString();
    }
    ...
    /**
     * Short-circuit match recognition that looks at the set of demographics components in the
     * expression and considers whether or not a value has been set for each non-existent or
     * unknown-matchable component. For this method to return true, each demographic component
     * must either 1a) not be specified; or 1b) specified as unknown matches (?); and 2) have a
     * null value. Each individual component must satisfy the above constraints. Use of this
     * method allows execution to avoid compiling the expressions and comparing them against a
     * collection of EJBs.
     * @return true if the above description is satisfied
     */
