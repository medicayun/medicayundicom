...
public class PatientMatchingTest extends TestCase {
	...
	public void test_ctor_shouldParseAlternateMatch_whenDemographicsInitialSpecifiedWithShortFormExpression() {
		String expression = "pid,issuer,familyname|familyname(1)";
    	PatientMatching pm = new PatientMatching(expression);
    	assertEquals("Rebuilt expression does not match the original", expression, pm.toString());
	}
	
	public void test_ctor_shouldParseAlternateMatch_whenDemographicsInitialSpecifiedWithLongFormExpression() {
		String expression = "pid,issuer,familyname|pid,issuer,familyname(1)";
		String expected = "pid,issuer,familyname|familyname(1)";
    	PatientMatching pm = new PatientMatching(expression);
    	assertEquals("Rebuilt expression does not match the original", expected, pm.toString());
	}
	
	public void test_ctor_shouldParseMultipleAlternateMatch_whenDemographicsInitialSpecifiedWithShortFormExpression() {
		String expression = "pid,issuer,familyname|familyname(1)|givenname(1),birthdate";
    	PatientMatching pm = new PatientMatching(expression);
    	assertEquals("Rebuilt expression does not match the original", expression, pm.toString());
	}
	
	public void test_ctor_shouldParseMultipleAlternateMatch_whenDemographicsInitialSpecifiedWithLongFormExpression() {
		String expression = "pid,issuer,familyname|pid,issuer,familyname(1)|pid,issuer,givenname(1),birthdate";
		String expected = "pid,issuer,familyname|familyname(1)|givenname(1),birthdate";
    	PatientMatching pm = new PatientMatching(expression);
    	assertEquals("Rebuilt expression does not match the original", expected, pm.toString());
	}
	
	public void test_ctor_shouldParseAlternateMatch_whenDemographicsInitialSpecifiedWithShortFormExpressionWithOptionalPidAndIssuer() {
		String expression = "pid?,issuer?,familyname|familyname(1)";
    	PatientMatching pm = new PatientMatching(expression);
    	assertEquals("Rebuilt expression does not match the original", expression, pm.toString());
	}
	
	public void test_ctor_shouldParseAlternateMatch_whenDemographicsInitialSpecifiedWithLongFormExpressionWithOptionalPidAndIssuer() {
		String expression = "pid?,issuer,familyname|pid?,issuer,familyname(1)";
		String expected = "pid?,issuer,familyname|familyname(1)";
    	PatientMatching pm = new PatientMatching(expression);
    	assertEquals("Rebuilt expression does not match the original", expected, pm.toString());
	...
	public void test_ctor_shouldThrow_whenParseAlternateMatchAndPidAndIssuerDoNotMatch() {
		Exception e = ExceptionTrap.capture(new ExceptionTrigger() {
			public void trip() {
				String s = "pid?,issuer,familyname|pid,issuer?,familyname(1)";
				new PatientMatching(s);
			}
		});
		assertEquals("Unexpected exception encountered", IllegalArgumentException.class, e.getClass());
	...
	public void test_extractAltExpression_shouldAddPidAndIssuer_whenItDoesNotExistInAlternateExpression() {
		String expression = "pid,issuer,familyname|familyname(1)";
		String expected = "pid,issuer,familyname(1)";
    	PatientMatching pm = new PatientMatching("pid,issuer"); // doesn't matter for this test, not used
    	assertEquals("Rebuilt expression does not match the original",
    			expected, pm.extractAltExpression(expression.indexOf('|'), expression));
	}
	
	public void test_extractAltExpression_shouldNotDuplicatePidAndIssuer_whenItExistsInAlternateExpression() {
		String expression = "pid,issuer,familyname|pid,issuer,familyname(1)";
		String expected = "pid,issuer,familyname(1)";
    	PatientMatching pm = new PatientMatching("pid,issuer"); // doesn't matter for this test, not used
    	assertEquals("Rebuilt expression does not match the original",
...
//	}
	...
	public void test_toString_shouldAppendWithComma_whenSexAppendedToBirthDateInDemographics() {
	    String s = "pid,issuer,ignore(\" |-|\\.|,|'|JR|SR\"),familyname,givenname(1),birthdate,sex";
	    PatientMatching pm = new PatientMatching(s);
	    assertEquals(pm.toString(), s);
	}

	public void test_toString_shouldAppendWithComma_whenBirthdateAppendedToSexInDemographics() {
	    String s = "pid,issuer,ignore(\" |-|\\.|,|'|JR|SR\"),familyname,givenname(1),sex,birthdate";
	    String s1 = "pid,issuer,ignore(\" |-|\\.|,|'|JR|SR\"),familyname,givenname(1),birthdate,sex";
	    PatientMatching pm = new PatientMatching(s);
	    // toString reconstructs with birth date and sex in a specific order,
	    // regardless of the original order in which they were provided
	    assertEquals(pm.toString(), s1);
	}
