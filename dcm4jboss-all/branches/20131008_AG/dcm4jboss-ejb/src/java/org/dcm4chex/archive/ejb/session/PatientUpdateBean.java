 ...
 * @version $Revision: 18387 $ $Date: 2014-11-04 04:06:29 +0800 (周二, 04 11月 2014) $
 * @since 14.01.2004
 * 
 * @ejb.bean
 *  name="PatientUpdate"
...
public abstract class PatientUpdateBean implements SessionBean {
    ...
    public void mergePatient(Dataset dominant, Dataset prior,
        ...
        PatientLocal dominantPat = null;
        try{
            dominantPat = updateOrCreate(dominant, matching);            
        } catch(PatientMergedException pme){
            // Cannot do patient merge when the dominant patient has been merged to another patient
            if(LOG.isDebugEnabled()){
                LOG.debug("Cannot proceed with patient merge, as dominant patient " 
                        + dominant.getString(Tags.PatientName)
                        + " with PID " + toPidAndIssuer(dominant)
                        + " has already been merged to another patient.");
            }
            throw pme;
        }
 
        PatientLocal priorPat = null;
        try{
            priorPat = updateOrCreate(prior, matching);
        } catch(PatientMergedException pme){
            // Return normally, when it's a duplicated merge request
            // I.E., prior patient has been merged to the dominant 
            priorPat = patHome.selectPatient(prior, matching, true);
            if(priorPat.isIdentical(dominantPat)){
                // return normally
                if(LOG.isDebugEnabled()){
                    LOG.debug("Ignoring duplicate patient merge, as prior patient " 
                            + prior.getString(Tags.PatientName)
                            + " with PID " + toPidAndIssuer(prior)
                            + " has already been merged to dominant patient " 
                            + dominant.getString(Tags.PatientName)
                            + " with PID " + toPidAndIssuer(dominant));
                }
                return;
            ...
            // Otherwise, throw the caught PatientMergedException
            if(LOG.isDebugEnabled()){
                LOG.debug("Cannot proceed with patient merge, as prior patient " 
                        + prior.getString(Tags.PatientName)
                        + " with PID " + toPidAndIssuer(prior)
                        + " has already been merged to another patient " + priorPat.getPatientName()
                        + " with PID " + toPidAndIssuer(priorPat.getPatientId(), priorPat.getIssuerOfPatientId()));
            }
            throw pme;
        }
   ...
   private PatientLocal updateOrCreate(Dataset ds, Dataset modified, PatientMatching matching)
    ...
    private PatientLocal findAndUpdatePatient(Dataset ds, Dataset modified,
    	...
    	// PID Issuer should only be updated via processing a 
    	// changePatientIdentifierList msg, so overwriting it with the value
    	// from database before doing the patient update 
    	ds.putLO(Tags.IssuerOfPatientID, pat.getIssuerOfPatientId());
    ...
    /**
     * Method to get the combined string of patient ID and Issuer with given argument. 
     * Sample of the returned result: PID1^^^Issuer1
     * 
     *
     * @param ds 
     *      dataset instance containing patient ID and issuer of patient ID
     * @return combined string of patient ID and Issuer
     */
    private String toPidAndIssuer(Dataset ds) {
        return this.toPidAndIssuer(ds.getString(Tags.PatientID), ds.getString(Tags.IssuerOfPatientID));
    }
  
    /**
     * Method to get the combined string of patient ID and Issuer with given arguments. 
     * Sample of the returned result: PID1^^^Issuer1
     *
     * @param patientId 
     *      patient ID
     ...
     * @return combined string of patient ID and Issuer
     */
    private String toPidAndIssuer(String patientId, String issuer) {
        return  patientId + "^^^" + (issuer == null ? "" : issuer);
    }
