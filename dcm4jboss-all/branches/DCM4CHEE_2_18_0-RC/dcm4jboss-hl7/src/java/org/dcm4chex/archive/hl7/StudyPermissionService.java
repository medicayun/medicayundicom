...
public class StudyPermissionService extends ServiceMBeanSupport {
    ...
    private boolean updateOnIanNotification;
    ...
    public void setIanScuServiceName(ObjectName ianScuServiceName) {
    ...
    public String getIanStylesheet() {
        return ianStylesheet;
    }

    public void setIanStylesheet(String ianStylesheet) {
        this.ianStylesheet = ianStylesheet;
    }
    ...
    public boolean isUpdateOnIanNotification() {
        return updateOnIanNotification;
    }

    public void setUpdateOnIanNotification(boolean updateOnIanNotification) {
        this.updateOnIanNotification = updateOnIanNotification;
    }
    ...
    protected void startService() throws Exception {
        ...
        server.addNotificationListener(ianScuServiceName,
                ianNotificationListener , IANScuService.NOTIF_FILTER, null);
    }
    ...
    private final NotificationListener ianNotificationListener =
            new NotificationListener() {
        public void handleNotification(Notification notif, Object handback) {
            StudyPermissionService.this.onIanNotification((Dataset) notif
                    .getUserData());
        }
    };
    ...
    private void onHl7Received(Document msg) {
        log.debug("HL7 notification received!");
        if (!updateOnHl7Received) return;
        try {
            if (log.isDebugEnabled())
                log.debug("Handle HL7 notification with stylesheet "+hl7Stylesheet);
    ...
    private void onSeriesStored(SeriesStored stored) {
        log.debug("SeriesStored notification received!");
    ...
    private void onMppsCreate(Dataset ds) {
        log.debug("MPPS Create notification received!");
    ...
    private void onIanNotification(Dataset ds) {
        log.debug("IAN notification received!");
        if (!updateOnIanNotification) {
            return;
        }
        try {
            xslt(ianStylesheet, ds, ds.getString(Tags.PerformedStationAET));        
        } catch (Exception e) {
            log.error("Failed to update permissions on IAN Notification", e);
        }
    }
    
    private void onNewStudy(Dataset ds) {
        log.debug("NewStudy notification received!");
        if (!updateOnNewStudy) {
            return;
        }
        try {
            String callingAET = PrivateTags.getStringValue(ds, PrivateTags.CallingAET);
            if (callingAET == null)
                callingAET ="UNKNOWN";
    ...
    private void xslt(String xslt, Dataset ds, String calling)
            throws Exception {
        if (log.isDebugEnabled()) {
            log.debug("Handle dataset with stylesheet "+xslt+". calling:"+calling+"! ds:");log.debug(ds);
        }
    ...
    private void logSecurityAlert(boolean success, String desc) {
        if (log.isDebugEnabled()) {
            log.debug(desc+" Success:"+success);
        }
